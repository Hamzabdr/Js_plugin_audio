...
app.get('/api/plugins', function(req, res) {
    ...
    let message = "<h1>/api/plugins: on demande des plugins";
...
app.get('/api/plugins/:id', function(req, res) {
    ...
    // On simule une réponse
    let message = "<h1>/api/plugins/" + id + " : on demande "
    message += " le détail du plugin par son id=" + id +"</h1>";
...
app.post('/api/plugins', multerData.fields([]), function(req, res) {
	...
	// Normalement on devrait renvoyer l'id du plugin créé
	// Là on renvoie juste les paramètres d'appel
	   // On simule une réponse
    let message = "<h1>Création d'un plugin avec les paramètres" 
...
app.put('/api/plugins/:id', multerData.fields([]), function(req, res) {
    ...
    let message = "<h1>Modification du plugin id=" + id + " avec les paramètres" 
    message += " nom = " + req.body.nom + " et cuisine= " + req.body.cuisine;
...
app.delete('/api/plugins/:id', function(req, res) {
    ...
    let message = "<h1>Suppresion du plugin id=" + id + "</h1>";
